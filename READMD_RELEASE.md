...
リポジトリのルートで実行する。`0.1.6` は公開済み。
以下は次のバージョン `0.1.7` を公開する例。
...
`tetorica-vgm-0.1.7.tgz` がリポジトリのルートに生成される。
...
```sh
tar -tzf tetorica-vgm-0.1.7.tgz
tar -xOf tetorica-vgm-0.1.7.tgz package/README.md
tar -xOf tetorica-vgm-0.1.7.tgz package/package.json
```
...
```sh
release_tarball="$PWD/tetorica-vgm-0.1.7.tgz"
...
```sh
npm publish ./tetorica-vgm-0.1.7.tgz --access public --registry https://registry.npmjs.org/ --dry-run
```
...
```sh
npm publish ./tetorica-vgm-0.1.7.tgz --access public --registry https://registry.npmjs.org/
```
...
npm view tetorica-vgm@0.1.7 dist.integrity --registry https://registry.npmjs.org/
...
(
  ...
  npx --yes tetorica-vgm@0.1.7 --help
  npx --yes tetorica-vgm@0.1.7 render "$release_nes_input" --output nes.wav
)
```
...
MAME HuC6280 support is not included in the already-published `0.1.7` package.
