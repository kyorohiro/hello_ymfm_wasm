...
`tetorica-vgm-0.1.8.tgz` がリポジトリのルートに生成される。
...
```sh
tar -tzf tetorica-vgm-0.1.8.tgz
tar -xOf tetorica-vgm-0.1.8.tgz package/README.md
tar -xOf tetorica-vgm-0.1.8.tgz package/package.json
```
...
```sh
release_tarball="$PWD/tetorica-vgm-0.1.8.tgz"
...
```sh
npm publish ./tetorica-vgm-0.1.8.tgz --access public --registry https://registry.npmjs.org/ --dry-run
```
...
```sh
npm publish ./tetorica-vgm-0.1.8.tgz --access public --registry https://registry.npmjs.org/
```
...
npm view tetorica-vgm@0.1.8 dist.integrity --registry https://registry.npmjs.org/
...
(
  ...
  npx --yes tetorica-vgm@0.1.8 --help
  npx --yes tetorica-vgm@0.1.8 render "$release_nes_input" --output nes.wav
)
```
