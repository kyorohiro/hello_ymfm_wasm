...
リポジトリのルートで実行する。`0.1.3` は公開済み。
以下は次のバージョン `0.1.4` を公開する例。
...
`tetorica-vgm-0.1.4.tgz` がリポジトリのルートに生成される。
...
```sh
tar -tzf tetorica-vgm-0.1.4.tgz
tar -xOf tetorica-vgm-0.1.4.tgz package/README.md
tar -xOf tetorica-vgm-0.1.4.tgz package/package.json
```
...
```sh
release_tarball="$PWD/tetorica-vgm-0.1.4.tgz"
...
```sh
npm publish ./tetorica-vgm-0.1.4.tgz --access public --registry https://registry.npmjs.org/ --dry-run
```
...
```sh
npm publish ./tetorica-vgm-0.1.4.tgz --access public --registry https://registry.npmjs.org/
```
...
npm view tetorica-vgm@0.1.4 dist.integrity --registry https://registry.npmjs.org/
...
(
  ...
  npx --yes tetorica-vgm@0.1.4 --help
  npx --yes tetorica-vgm@0.1.4 render "$release_nes_input" --output nes.wav
)
```
