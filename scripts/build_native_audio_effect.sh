...
mkdir -p "$OUT_DIR"
emcc "$ROOT_DIR/native/audio_effect/noise.c" "$ROOT_DIR/native/audio_effect/gain.c" "$ROOT_DIR/native/audio_effect/reverb.c" "$ROOT_DIR/native/audio_effect/compressor.c" "$ROOT_DIR/native/audio_effect/noise_gate.c" "$ROOT_DIR/native/audio_effect/eq.c" "$ROOT_DIR/native/audio_effect/graph.c" "$ROOT_DIR/native/audio_effect/extra_fx.c" -O2 -sALLOW_MEMORY_GROWTH=1 -sINITIAL_MEMORY=33554432 -sSTANDALONE_WASM=1 \
