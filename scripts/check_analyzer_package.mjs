...
function check(file, specifier) {
  ...
  checked++;
  if (relative(root, target).startsWith(`..${sep}`) || !existsSync(target) || !statSync(target).isFile())
...
function visit(dir) {
  for (const entry of readdirSync(dir, {withFileTypes:true})) {
    ...
    for (const match of source.matchAll(/\b(?:from\s*|import\s*\(?\s*|new\s+URL\s*\(\s*)["']([^"']+)["']/g)) check(file, match[1]);
