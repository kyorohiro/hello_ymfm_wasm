...
struct sample_interface : ymfm_wasm_interface
{
    ...
    { return type == ymfm::ACCESS_ADPCM_B && address < memory.size() ? memory[address] : 0; }
    void ymfm_external_write(ymfm::access_class type, uint32_t address, uint8_t data) override
    { if (type == ymfm::ACCESS_ADPCM_B && address < memory.size()) memory[address] = data; }
};
...
struct y8950_handle
{
    ...
    y8950_handle() : intf(), chip(intf)
    {
        chip.reset();
    }
};

inline y8950_handle *cast_handle(void *ptr)
{
    return static_cast<y8950_handle *>(ptr);
}
...
void *y8950_create()
{
    return new y8950_handle();
}

void y8950_destroy(void *ptr)
{
    delete cast_handle(ptr);
}

void y8950_reset(void *ptr)
{
    cast_handle(ptr)->chip.reset();
}

void y8950_write(void *ptr, uint32_t offset, uint8_t data)
{
    cast_handle(ptr)->chip.write(offset, data);
}

uint8_t y8950_read(void *ptr, uint32_t offset)
{
    return cast_handle(ptr)->chip.read(offset);
}

uint8_t y8950_read_status(void *ptr)
{
    return cast_handle(ptr)->chip.read_status();
}

uint8_t y8950_get_irq(void *ptr)
{
    return cast_handle(ptr)->intf.irq_asserted ? 1 : 0;
}

uint32_t y8950_sample_rate(void *ptr, uint32_t clock)
{
    return cast_handle(ptr)->chip.sample_rate(clock);
}

int y8950_load_memory(void *ptr, const uint8_t *data, uint32_t size, uint32_t offset, uint32_t memory_size)
{
    if (memory_size > 2097152 || offset > memory_size || size > memory_size - offset) return 0;
...
void y8950_generate(void *ptr, float *left, float *right, uint32_t frames)
{
    ...
    {
        ymfm::y8950::output_data output;
        handle->chip.generate(&output);
        handle->intf.advance(72);
