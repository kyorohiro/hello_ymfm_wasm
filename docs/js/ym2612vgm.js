...
export function exportYm2612VgmToPlaygroundJavaScript(source, options = {}) {
  ...
  if (options.includeHeaderComment !== false) {
    ...
    lines.push("// Timing uses VGM sample units at 44100 Hz via sleepSamples().");
    lines.push("");
  }
...
function renderPlaygroundTrack(track, totalLoopSamples) {
  ...
  for (const event of track.events) {
    ...
    if (delta > 0) {
      lines.push(`  await sleepSamples(${delta});`);
  ...
  if (tail > 0 || track.events.length === 0) {
    lines.push(`  await sleepSamples(${Math.max(1, tail)});`);
  }
  lines.push("});");
  return lines;
}
