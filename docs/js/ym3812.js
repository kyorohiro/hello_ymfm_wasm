export const YM3812_CLOCK = 3579545;

/**
 * Ym3812 chip instance backed by WASM. No AudioContext or playback device is created.
...
export class Ym3812 {
  ...
  static async create(options = {}) {
    ...
    const api = {
      create: module.cwrap("ym3812_create", "number", []),
      destroy: module.cwrap("ym3812_destroy", null, ["number"]),
      reset: module.cwrap("ym3812_reset", null, ["number"]),
      write: module.cwrap("ym3812_write", null, ["number", "number", "number"]),
      read: optionalCwrap(module, "ym3812_read", "number", ["number", "number"]),
    ...
    return new Ym3812(module, handle, api);
  }
  ...
  read(offset) {
    if (typeof this.api.read !== "function") {
      throw new Error("This YM3812 runtime does not support read(offset). Rebuild or reload the generated wasm runtime.");
    }
...
/**
 * Convenience factory for Ym3812. Does not create an audio device.
 ...
 * @returns {Promise<Ym3812>} Chip instance owned by the caller.
 */
export async function createYm3812(moduleFactory, moduleOptions) {
  return Ym3812.create({ moduleFactory, moduleOptions });
}
