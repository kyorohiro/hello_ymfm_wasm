...
単独・非隣接の周波数書き込みがあっても、TL等の音色状態は無効化しない。
周波数ラッチの追跡のみを無効化し、同じTLの再設定を除去する。
変更されたTLや各周回の初回設定は維持する。除去前後の待ち時間は絶対時刻から合算する。

目標はブラウザ・CPU・タイマー起床による実時間の完全一致ではなく、
元のサンプル時刻とKEY操作を保持した、読みやすく聴き比べ可能な出力とする。
テストは生成コードの論理サンプル時刻を比較し、実時間の一致を要求しない。

### PC-98 / Neo Geo FM対応

YM2203・YM2608・YM2610のネイティブFM出力にもHigh / Compactを適用する。
画面側でWriteへ固定していた分岐と、変換側のYM2612限定条件を修正。
ネイティブFM APIにはsetOperatorsがないため、順序を保ったsetOperator列を生成する。
SSG・Rhythm・ADPCM等の除外は既存方針を維持する。
Neo Geo入力をYM2612へ変換する経路も追加した。

Note-ishの音程基準はネイティブ出力ではチップのヘッダークロックを使用し、
YM2203の標準FM分周はYM2612の2倍の換算クロックで扱う。
YM2612互換変換後は変換先の基準クロックを使う。
可変プリスケーラー等の既存変換で扱わない特殊動作の完全再現は対象外。

3チップ・High/Compact・CH分割あり/なしの生成コードをネイティブSynthで実行し、
FM以外の書き込み除外とKEY時刻を検証。実音の聴き比べは未実施。
