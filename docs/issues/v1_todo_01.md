...
- [ ] Operator Info を追加する。
- [x] OPM / OPL3 / OPL4 の JSON レジスタスナップショットを Export に追加する。
- [ ] 2op / 4op、ペア構成、リズムの状態を確認できるようにする。

音色情報の表示・保存と、音色編集・他形式への音色変換は別の対応項目とする。

JSON は現在処理済みの VGM 時刻までの書き込みを再解析する。OPM は AMD/PMD・
ノイズ・キー状態も保持。OPL3/4 は両バンク・4op ペア・リズム・オペレーターを保存。
OPL4 PCM は明示的なレジスタ書き込みのみで、波形ヘッダーの自動ロード値と
サンプルメモリーは含まない。音声・エンベロープのセーブステートではない。
