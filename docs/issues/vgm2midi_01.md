> 更新: SSG／Sega PSGのトーン3chをMIDI出力に追加。最新仕様は末尾の「SSG／PSGトーン対応」を参照。
...
- YM2203／YM2608のSSG 3ch、Sega PSGのトーン3chをMIDIトラックに追加。
  FMが発音しないファイル・PSG単独ファイルでも書き出せる。
- FMトラックに続いてSSG／PSGトラックを配置。MIDIの打楽器用10chを避ける。
- `tone_notes.js` でレジスタから音高・発音状態を共通計算し、再生モニターとMIDI解析に使用。
- SSG: トーン有効かつ固定音量非0／エンベロープ有効を発音とみなす。
  音量0・トーン無効で終了し、エンベロープ形状書込みは打ち直しとする。
- SSG周期0は1として扱う。ymfmの実装に従い、実効SSGクロックは通常YM2203入力/2、
  YM2608入力/4。プリスケーラー変更も反映。
- PSG: ラッチ・データ書込みを追跡し、減衰値15で終了。周期0はSega PSGの1024として扱う。
  SN76489派生チップの全差分・ステレオマスク・2台目は対象外。
- 発音中の周期変更は同一ノートのPitch Bend。ノイズ専用出力はノートにしない。
  SSGでトーンとノイズを混ぜた場合はトーン音高を出力し、警告を付ける。
- SSGエンベロープ位相・音量推移・自動消音は再現しない。エンベロープ使用音は
  ミキサー／音量の明示的な消音まで継続するため、音長が実音より長くなる場合がある。
- FMは従来の1種類選択を維持。YM2612→YM2608→YM2203の優先順。
  MIDIでは選択したOPNのSSGと、ヘッダーで宣言されたPSGを加える。
- MIDI、SSG／PSGモニター、指板、MMLの関連43テスト通過。
  実曲のブラウザー表示・DAW読込は未確認（接続可能なブラウザーなし）。

確認先: 同梱 `src/ymfm_opn.h` の `ssg_effective_clock()`、`src/ymfm_ssg.cpp`、
`src/segapsg.cpp`、[VGM Specification](https://vgmrips.net/wiki/VGM_Specification)。
