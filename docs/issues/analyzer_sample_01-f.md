...
## RF5C164 / YM2612 DAC 初期実装

RF5C164は発音時点の64 KiB RAMスナップショット、開始位置、ループ先、step、音量、パンを記録する。チャンネル無効化を終了として記録し、全体無効化は一時停止として扱う。生データ保存はサンプル領域だけではなくRAM全体である。再生途中のRAM変更・設定変更の試聴再現は未対応。

YM2612 DACは時刻付き出力記録として追加した。既存パーサーのplayStep / consumeWaitを利用し、直接レジスタ書き込み、PCMバンク読み出し、対応済みDACストリーム制御を同じ出力へ記録する。

- 区切りはDAC無効化、曲末、10秒ごとの表示用区間。元の楽器サンプル境界を検出したものではない。
- 保存はJSON。44100 Hz基準の開始時刻・相対書き込み時刻・バイト列・区間長・区切り理由を含む。
- 試聴は書き込み間隔を保持したゼロ次ホールド、中央定位。実チップのアナログ特性、フィルタ、パンは再現しない。
- 無効中の書き込み値も保持し、DAC再有効化時の初期値へ反映する。
- 同時刻の書き込みを保存し、試聴ではその時刻の最後の値を使用する。
- 保持上限は400万書き込み。曲ループは展開しない。
- PCMバンクの元領域への逆参照、楽器単位の重複統合、WAV保存は未対応。

RF5C164のバンク・RAM世代・停止／再開、およびDACの不等間隔・同時刻書き込み、表示区間分割、PCMバンク、ストリーム時刻をテストする。
