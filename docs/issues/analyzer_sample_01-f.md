...
| 2 | RF5C164（Mega-CD / Sega CD） | 時系列で変化するPCM RAM、8チャンネル |
| 2（RF5C164の後） | YM2612 DAC | CH6のDACへ送られるPCMデータと書き込み時刻 |
...
RF5C164 PCM、YM2612 DAC、Rhythm、発音途中の設定変更、自然終了追跡、波形、タイムラインへの移動、WAV / MIDI / MML連携は未実装。

## 追加予定: YM2612 DACのサンプル抽出

Mega DriveのYM2612 DACもSample Explorerの対象に含める。Mega-CDのRF5C164とは別音源として扱う。両者ともPCMを扱い、ADPCM音源とは区別する。

- YM2612 DAC: CH6を使うPCM出力。DACへのデータ書き込みと、その時刻を追跡する。
- RF5C164: PCM RAMを使う8チャンネル音源。再生位置・ループ・時系列のRAM書き換えを追跡する。
- YM2203: FM 3チャンネルとSSG 3チャンネルのみで、チップ本体のADPCM / PCM抽出対応は不要。

YM2612 DACの解析では、VGMのPCMデータバンクを利用する再生と、DACレジスタへの直接書き込みを対象にする。ストリーム制御による再生も追跡し、元データの参照位置・使用時刻・レート・DAC有効状態を保持する。

DACにはサンプルの開始・終了アドレスを指定する仕組みがないため、ADPCMのアドレス範囲モデルをそのまま適用しない。VGMのストリーム情報などから範囲を特定できる場合はサンプル定義へ関連付ける。直接書き込みなどで境界を特定できない場合は、時刻付きPCM列として保持し、区間分割を推定した場合はその根拠を明示する。

書き込み間隔が一定とは限らないため、単一の再生レートを仮定して元の時刻情報を捨てない。一覧・使用履歴・単体試聴・生データ保存へ接続し、WAV出力は別途追加する。
