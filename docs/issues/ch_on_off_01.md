...
| YM2203 | FM 3CH | SSG 全体 | CH Off はコアで出力だけを抑制する方式へ変更済み |
...
| AY8910 / YM2149 | AY 3CH（再生画面の共通欄） | AY 全体、併用 OPLL 全体 | AY のミュートマスクをコアへ渡す。OPLL は複数チップミキサーで制御 |
...
| YM2413 | 単独再生の9CHボタン追加済み | 併用 Sega PSG 全体 | リズムは CH7～9 のグループ単位。MSX 複合 UI にもCH接続済み |
| YM3526、YM3812 | 対象チップ自身の CH 別 UI は未対応 | 併用 Sega PSG 全体の API あり | `setPsgMuted` は対象 FM チップをミュートする機能ではない |
| Y8950 | FM 9CH | Y8950 全体・ADPCM・併用 Sega PSG | 出力だけを抑制。リズムは CH7～9 のグループ単位 |
| YMF278B | FM / PCM CH 別 UI は未対応 | 併用 Sega PSG 全体の API あり | OPL4 自身の FM / PCM 切り替えは未対応 |
| MSX 複合構成 | AY 3CH・OPLL 9CH・Y8950 9CH | 各チップ全体・Y8950 ADPCM | 搭載チップのみ共通欄に表示。第1インスタンスが対象 |

「Play only」は解析・編集画面の対応範囲を示すもの。
CH 解析モニターが未対応でも、対応済み音源には独立して共通の ON/OFF ボタンを生成する。
...
- YM2203 の旧方式はキーオンを抑制していた。現在は出力マスクへ変更し、この制約を解消した。
- YM2612 CH6 は DAC 値を `0x80` に置き換える。ミュート中の最後の元データを保持して復元する方式ではない。
- FM のパン制御は元の値をモニターに保持して復元する。チップの汎用 CH ミュート API ではない。
- AY 専用モニターのハンドラーも、音源 API 呼び出し後に `flushPendingAudio()` で生成済み音声 queue を更新する。
...
- [x] MSX 複合エンジンの既存ミュート API を画面へ接続する（第1インスタンス）。
...
既存の CH ボタンと SSG / ADPCM-A / ADPCM-B の切り替えを確認した。
今回は音源機能の追加や WASM の再ビルドは行わず、回帰テストを補強。

- 通常 YM2610 の4FM CH、YM2610Bの6FM CHそれぞれで、既存のパン消音・解除後の波形一致を確認。
- SSG / ADPCM-A / ADPCM-B は両バリアントで、ミュート中のレジスタ変更と解除後の波形一致を確認。
- 音源名、UI の音源別 API 対応、全音源ミュート判定に YM2610 のケースを追加。
- 内部の保持サンプルがあるため、切り替え直後の128ネイティブサンプルを進めた後に波形を比較する。
  操作した瞬間からの完全な無遅延消音を証明するものではない。

[実WASMテスト](../playground/ym2610b_audio.test.mjs) と
[音源別操作テスト](../vgm_analyzer/source_mutes.test.mjs) 等、関連11件成功。
ADPCM-A の6音や SSG の3CHをそれぞれ独立に切り替える機能は今回追加していない。
ブラウザーでの実操作・実曲は未確認。次は YM2203。


## A 優先：YM2203 の方式変更

FM 3CHのミュートをコア出力マスクへ変更し、Analyzer のキーオフ強制・キーオン抑制を撤去。
ミュート中もフィードバックを含めて計算し、解除後に持続音を復帰できるようにした。
既存 CH ボタン・queue flush・SSG 全体切り替えは維持。リセット後もマスクを再適用する。
WASM 再ビルドと `docs/js/` 同期済み。

[実音源テスト](../playground/ym2203_audio.test.mjs) で3CHの消音、ミュート中のキーオン／オフ、
解除後の波形一致を確認。[UI 接続テスト](../vgm_analyzer/opn_mute.test.mjs) で元のキーオン値が配送されることを確認。
SSG 等の回帰を合わせて11テスト成功。保持サンプルを進めてから波形比較しており、即時無遅延の保証ではない。
複数音源の比較中に初期化 options の再利用問題も見つかり、WASM factory へ浅いコピーを渡すよう修正。
ブラウザー実操作・実曲は未確認。次は YM2149。


## YM2149 / AY8910 の検証と queue 更新

- 専用モニターの A/B/C・AY 全体・併用 OPLL 全体の操作後に `flushPendingAudio()` を呼ぶようにした。
- 実WASMで AY8910 / YM2149 × 3CH × トーン・ノイズ・エンベロープの18パターンを検証。
  ミュート中もレジスタを書き換え、解除後の波形が常時演算した参照エンジンと一致することを確認した。
- 全体ミュート解除時のCHマスク保持と、リセット後のミュート保持も確認。
- 音量ゼロにもDCオフセットがあるため、単一CHの出力確認では他CHを明示的にミュートする。
- UI ハンドラーの配送先と queue 更新は Node VM で実コードを検証。音源コアの変更は不要だった。

```sh
node --test web/ay8910.test.mjs docs/vgm_analyzer/ay_mute.test.mjs
```

関連12テスト成功。ブラウザー実操作・実曲は未確認。MSX 複合画面の共通CHボタンは対象外。
次は B 優先の Y8950。


### AY 操作欄の統一

AY8910 / YM2149 の ON/OFF を再生画面の共通欄へ移動した。
AY 全体・CH A/B/C・併用時の YM2413 全体を表示し、Operator タブはレジスタ状態の表示に専念する。
リセット時はミュート状態を維持し、曲の読み込み時は解除する。
共通ボタンの表示・状態保持・API配送・queue更新と既存音源テストの計16件成功。
ブラウザーでの実操作は未確認。


## Y8950 / MSX：今回の ON/OFF 拡張の区切り

S・A と MSX（Y8950 を含む）を今回の範囲とし、残りのチップへの拡張は保留する。

- Y8950：FM 9CH、ADPCM、チップ全体を共通欄で切り替える。併用 Sega PSG があれば全体ボタンも表示。
- リズム：CH7 は BD、CH8 は HH/SD、CH9 は TOM/CYM。5楽器個別の切り替えではない。
- コアの出力マスクは FM 9bit と ADPCM 1bit。ミュート中もFMフィードバックとADPCMの時間を進める。
- MSX：搭載されている AY・YM2413・Y8950 のCH別・全体とY8950 ADPCMを表示。
  `msx_mutes.js` が実際のエンジン登録先へ配送する。全体解除後も個別マスクを保持する。
- 操作時は生成済み音声 queue を更新。曲の読み込みで設定をクリアし、リセット・シーク時は保持。
- 第2インスタンスの UI は今回の対象外。ブラウザー実操作・実曲は未確認。

追加テスト：

- [Y8950 実WASM・MSX結合テスト](../../web/y8950-mute.test.mjs)：FM9CH、リズム3グループ、ADPCM、解除後の波形一致、リセット、音源分離、実MSXエンジンへの配送。
- [MSX UIテスト](../vgm_analyzer/msx_mutes.test.mjs)：搭載チップによる表示、登録先解決、共通欄とインライン欄の操作・queue更新。

AY・既存CHボタンの回帰を含め23テスト成功。配信用JSとY8950 WASMも更新済み。
