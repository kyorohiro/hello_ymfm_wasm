...
export function installMidiImport({button, presets, importFiles, onError, enabled}) {
  ...
  button.title=enabled?'Import a MIDI file; choose FM / PSG parts and voices':'MIDI import requires YM2612 (ymfm) mode';
    ...
    try {
      ...
      confirm.onclick=()=>{
        try {
          ...
          importFiles(file.name,bytes,routes);dialog.close();
