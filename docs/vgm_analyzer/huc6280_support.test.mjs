...
test('HuC6280 opens Parsed Output and disables unavailable analysis while switching back restores tabs',()=>{
  ...
  context.updateChipSupport();assert.equal(context.selected,'parsed-output');
  for(const key of ['operatorInfoTab','noteishTab','tfiInfoTab','sampleTab','sheetMusicTab','exportMidiButton','exportMmlButton'])assert.equal(context[key].disabled,true,key);
