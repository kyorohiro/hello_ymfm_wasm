...
test('AY monitor shows written registers and restores mute controls on file changes',()=>{
  ...
  try{
    ...
    monitor.reset();assert.equal(monitor.controls().find(c=>c.key==='0').muted,true);
    monitor.load({ay8910Clock:1789773,ay8910Type:16});assert.equal(monitor.controls().find(c=>c.key==='0').muted,false);
    assert.equal(root.children[0].textContent,'YM2149');assert.equal(monitor.controls().some(c=>c.key==='opll'),false);
  }finally{globalThis.document=before;}
});
test('Analyzer AY mode enables monitor, notes and MIDI/MML while disabling FM patches',()=>{
  ...
  const notice={},context={...chipSupportContext(),currentChipKind:'ay8910',opnMonitorRoot:{},ayMonitorRoot:{},document:{getElementById:()=>notice},selected:null,setOutputTab(name){context.selected=name;}};
  ...
  assert.equal(context.noteishTab.disabled,false);assert.equal(context.selected,'operator-info');
  ...
  assert.equal(context.exportMidiButton.disabled,false);assert.equal(context.exportMmlButton.disabled,false);
  assert.equal(context.exportAllTfiButton.disabled,true);assert.equal(context.exportAllVgiButton.disabled,true);
