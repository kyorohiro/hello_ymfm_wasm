/** Node.js で YM2612 のCH3 specialで4 Operatorの周波数を独立設定してWAVに保存する。 */
...
async function main() {
  ...
  try {
    ...
    frequencies.forEach((hz, operator) => {
      ...
      const { block, fnum } = hzToBlockFnum(hz, YM2612_CLOCK);
    ...
    const output = process.argv[2] ?? new URL("./ym2612_3chsp.wav", import.meta.url);
