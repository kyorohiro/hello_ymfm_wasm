/** Node.js で YM2608 のCH3 specialで4 Operatorの周波数を独立設定してWAVに保存する。 */
...
async function main() {
  const chip = await Ym2608.create({
    moduleFactory,
    moduleOptions: { wasmBinary: await readFile(new URL("../../docs/generated/ym2608_wasm.wasm", import.meta.url)) },
  });
  ...
  try {
    ...
    frequencies.forEach((hz, operator) => {
      ...
      const { block, fnum } = hzToBlockFnum(hz, YM2608_CLOCK);
    ...
    const output = process.argv[2] ?? new URL("./ym2608_3chsp.wav", import.meta.url);
