/** Node.js で YM2203 のCH3 specialで4 Operatorの周波数を独立設定してWAVに保存する。 */
...
async function main() {
  const chip = await Ym2203.create({
    moduleFactory,
    moduleOptions: { wasmBinary: await readFile(new URL("../../docs/generated/ym2203_wasm.wasm", import.meta.url)) },
  });
  ...
  try {
    ...
    frequencies.forEach((hz, operator) => {
      ...
      const { block, fnum } = hzToBlockFnum(hz, YM2203_CLOCK * 2);
    ...
    const output = process.argv[2] ?? new URL("./ym2203_3chsp.wav", import.meta.url);
