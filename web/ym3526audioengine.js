...
// YM3526 and optional Sega PSG share the output clock, but keep independent state.
/**
 * Ym3526AudioEngine adapter for synchronous stereo rendering and VGM register dispatch.
 ...
 */
export class Ym3526AudioEngine {
  /**
  ...
  static async create({ ym3526ModuleFactory, ym3526ModuleOptions, ym3526Clock = YM3526_CLOCK,
    ...
    if (!Number.isFinite(outputSampleRate) || outputSampleRate <= 0 ||
        !Number.isFinite(ym3526Clock) || ym3526Clock <= 0) throw new RangeError('Invalid sample rate or chip clock');
    const chip = await Ym3526.create({ moduleFactory: ym3526ModuleFactory, moduleOptions: ym3526ModuleOptions });
    let psg;
    try {
      if (psgClock) psg = await SegaPSG.create({ moduleFactory: segaPsgModuleFactory, clock: psgClock, sampleRate: outputSampleRate });
      return new Ym3526AudioEngine(chip, psg, chip.sampleRate(ym3526Clock), outputSampleRate, masterVolume);
   ...
   */
  reset() {
    this.ym3526.reset(); this.psg?.reset(); this.remainder = 0; this.lastLeft = 0; this.lastRight = 0;
  }
  /**
   * Release the underlying chips and their resources. Do not render after disposal.
   * @returns {void}
   */
  dispose() { this.ym3526.dispose(); this.psg?.dispose(); }
  /**
  ...
  process(left, right, frames) {
    ...
    for (let i = 0; i < frames; i++) {
      ...
      if (count) {
        const pcm = this.ym3526.generateStereo(count);
...
export const createYm3526AudioEngine = options => Ym3526AudioEngine.create(options);
