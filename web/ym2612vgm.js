...
function exportOpnFmVgmToPlaygroundJavaScript(source, options, chipKind) {
  ...
  const writeRegister = chipKind === "ym2608"
    ? createYm2608FmWriteTranslator(
...
function createYm2608FmWriteTranslator(sourceClock, writeRegister) {
  ...
  return (register, value, port = 0) => {
    if (!isYm2608FmRegister(port, register)) {
      return;
    }
