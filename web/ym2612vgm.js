...
export function exportYm2612VgmToPlaygroundJavaScript(source, options = {}) {
  ...
  while (true) {
    ...
    if (event.type === "ym2612-write") {
      const target = getYm2612WriteTarget(event.port, event.register, event.value);
      ...
      tracks[trackIndex].events.push({
        ...
        value: event.value,
        comment: describeYm2612Write(event.port, event.register, event.value),
      });
    }
    if (event.type === "end") {
      break;
  ...
  if (options.includeHeaderComment !== false) {
    lines.push("// Generated by the Tetorica FM2612 VGM Analyzer.");
    lines.push("// Register writes are scheduled at VGM sample positions (44100 Hz).");
    lines.push("");
  }
  ...
  for (let index = 0; index < renderOrder.length; index += 1) {
    ...
    lines.push(...renderPlaygroundTrack(track, totalLoopSamples));
 ...
 * @returns {string[]}
 */
function renderPlaygroundTrack(track, totalLoopSamples) {
  /** @type {string[]} */
  const lines = [
    `liveLoop(${JSON.stringify(track.name)}, async () => {`,
    "  const cycleStart = beginSampleSchedule();",
    "  scheduleWritesSamples(cycleStart, [",
  ];
  for (const event of track.events) {
    if (event.comment) {
      lines.push(`    // ${event.comment}`);
    }
    lines.push(`    [${event.timeSamples}, ${event.port}, ${formatHexNumber(event.register)}, ${formatHexNumber(event.value)}],`);
  }
  lines.push("  ]);");
